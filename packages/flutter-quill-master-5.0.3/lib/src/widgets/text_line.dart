...
class _TextLineState extends State<TextLine> {
  ...
  void _tapNodeLink(Node node) {
    ...
    final title = node.toPlainText();

    _tapLink(link, title);
  }

  void _tapLink(String? link, String? title) {
    if (link == null) {
      return;
    }
    ...
    // launchUrl(link);
    launchUrl(jsonEncode({"link":link,"title":title}));
  }
...
class EditableTextLine extends RenderObjectWidget {
  ...
  EdgeInsetsGeometry _getPadding() {
    return EdgeInsetsDirectional.only(
        start: indentWidth,end: indentWidth,
...
class RenderEditableTextLine extends RenderEditableBox {
  ...
  Offset _getOffset(Offset offset){
    ...
    var relativeCaretOffset = _body!.getOffsetForCaret(position, _caretPrototype);

    if(relativeCaretOffset.dx != 0){
      final parentData = _body!.parentData as BoxParentData;
      final effectiveOffset = offset + parentData.offset;
      relativeCaretOffset = relativeCaretOffset + effectiveOffset;
    }

    // final caretOffset = getOffse tForCaret(position);
    //
    // var caretRect = _caretPrototype.shift(caretOffset);
    //
    // final caretPosition = _body!.localToGlobal(caretRect.topLeft);
  ...
  void paint(PaintingContext context, Offset offset) {
    if (_leading != null) {
      ...
      // final effectiveOffset = offset + parentData.offset;
      final effectiveOffset =_getOffset(offset);
