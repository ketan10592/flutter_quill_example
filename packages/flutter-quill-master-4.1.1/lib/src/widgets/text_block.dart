...
class EditableTextBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _EditableBlock(
        ...
        decoration: _getDecorationForBlock(block, defaultStyles, ctx: context) ??
            const BoxDecoration(),
        contentPadding: contentPadding,
        children: _buildChildren(context, indentLevelCounts));
  }

  TextDirection  _getTextDirection(BuildContext? cntx, {Line? line}) {
    var style = line!.style;
    var attribute = style.attributes[Attribute.align.key] ??
        (Directionality.of(cntx!) == TextDirection.ltr ? Attribute.leftAlignment : Attribute.rightAlignment);
    if(attribute == Attribute.leftAlignment){
      return TextDirection.ltr;
    }else if(attribute == Attribute.rightAlignment){
      return TextDirection.rtl;
    }else{
      return Directionality.of(cntx!);
    }
  }

  BoxDecoration? _getDecorationForBlock(
      Block node, DefaultStyles? defaultStyles,{BuildContext? ctx}) {
    final attrs = block.style.attributes;
    if (attrs.containsKey(Attribute.blockQuote.key)) {
      if(attrs.containsKey(Attribute.align.key)){
        final directionAtr = attrs[Attribute.align.key];
        if(directionAtr == Attribute.rightAlignment){
  ...
  Widget? _buildLeading(BuildContext context, Line line, int index,
    ...
    if (attrs[Attribute.list.key] == Attribute.ol) {
      return QuillNumberPoint(
        ...
        width: 32,
        padding: 8,
        direction: _getTextDirection(context,line: line),
      );
    }
